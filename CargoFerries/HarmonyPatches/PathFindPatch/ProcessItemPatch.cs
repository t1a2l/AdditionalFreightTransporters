...
namespace CargoFerries.HarmonyPatches.PathFindPatch
{
    internal class ProcessItemPatch
    {
        ...
        {
            PatchUtil.Patch(new PatchUtil.MethodDefinition(typeof(PathFind), "ProcessItem", argumentTypes: FindProcessItemMethodArgTypes()),
                null, 
                null,
        ...
        {
            PatchUtil.Unpatch(new PatchUtil.MethodDefinition(typeof(PathFind), "ProcessItem", argumentTypes: FindProcessItemMethodArgTypes()));
        }
        
        private static Type[] FindProcessItemMethodArgTypes()
        {
            var methodInfo = typeof(PathFind).GetMethods(BindingFlags.Instance | BindingFlags.NonPublic).First(m => m.Name == "ProcessItem" && m.GetParameters().Length == 5) ?? throw new Exception("PathFind.ProcessItem() with 5 args not found!");
            Debug.Log("Barges - PathFind.ProcessItem() with 5 args is found");
            ...
            IEnumerable<CodeInstruction> instructions)
        {
            ...
            foreach (var codeInstruction in codes)
            {
                ...
                newCodes.Add(new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ProcessItemPatch), nameof(TreatSpecially))));
